...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "{{ config({'myconf': run_started_at - 1}) }}",
        ...
        (
            "{{ config(custom_config=var('my_variable')) }}",
            {'custom_config': "var('my_variable')"}
        ),
        # Custom config with env_var() - will be evaluated by dbt at runtime
        (
            "{{ config(custom_env=env_var('MY_ENV_VAR')) }}",
            {'custom_env': "env_var('MY_ENV_VAR')"}
        ),
        ...
        (
            "{{ config(custom_schema=target.schema + '_suffix') }}",
            {'custom_schema': "target.schema + '_suffix'"}
        ),
        ...
        (
            '{{ config(schema="my_schema", enabled=true) }}',
            {'schema': '"my_schema"', 'enabled': 'true'}
        ),
        # Double quotes with spaces
        (
            '{{ config(custom_config="value with spaces") }}',
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("config(value=func(nested([a, b]), {'key': 'val'}), next='item')", 13, (',', ')'), "func(nested([a, b]), {'key': 'val'})"),
